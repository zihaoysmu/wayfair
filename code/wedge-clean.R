...
write_csv(
  ...
  temp_csv_path("zipcode_emp_calibration_diagnostics_2015_2022")
)
...
required_zip_cols <- c(
  "online_estab", "local_estab", "warehouse_estab",
  "online_emp_hat", "local_emp_hat", "warehouse_emp_hat",
  "sales_tax", "cit", "population",
...
wedge_year <- zipcode_year_wedge |>
  ...
  summarise(
    ...
    warehouse_estab = sum(warehouse_estab, na.rm = TRUE),
    online_emp_hat = sum(online_emp_hat, na.rm = TRUE),
    local_emp_hat = sum(local_emp_hat, na.rm = TRUE),
    warehouse_emp_hat = sum(warehouse_emp_hat, na.rm = TRUE),
    payann = sum(payann, na.rm = TRUE),
    emp = sum(emp, na.rm = TRUE),
...
wedge_sum_check <- zipcode_year_wedge |>
  ...
  left_join(
    wedge_year |>
      select(
        wedge_id, year, online_estab, local_estab, warehouse_estab,
        online_emp_hat, local_emp_hat, warehouse_emp_hat,
        payann, emp, population
      ),
    by = c("wedge_id", "year"),
...
if (
    ...
    any(abs(wedge_sum_check$warehouse_estab_zip_sum - wedge_sum_check$warehouse_estab_wedge) > 1e-8) ||
    any(abs(wedge_sum_check$online_emp_hat_zip_sum - wedge_sum_check$online_emp_hat_wedge) > 1e-8) ||
    any(abs(wedge_sum_check$local_emp_hat_zip_sum - wedge_sum_check$local_emp_hat_wedge) > 1e-8) ||
    any(abs(wedge_sum_check$warehouse_emp_hat_zip_sum - wedge_sum_check$warehouse_emp_hat_wedge) > 1e-8) ||
    any(abs(wedge_sum_check$payann_zip_sum - wedge_sum_check$payann_wedge) > 1e-8) ||
    any(abs(wedge_sum_check$emp_zip_sum - wedge_sum_check$emp_wedge) > 1e-8) ||
