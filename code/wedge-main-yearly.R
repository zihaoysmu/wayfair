...
# 2. Keep only wedge pairs that are two-sided and present in every year.
# 3. Build treatment/control variables and the low/high tax-gap bins.
# 4. Run the same year-by-year models for the baseline and all-OLS outputs.
# 5. Save the CSV tables and PNG figures with the original file names.
...
save_plot(tax_gap_histogram, "wedge_tax_diff_histogram")
...
# Final output calls. Quadratic and lagged-treatment robustness outputs live in
# code/robust.R. The all-OLS versions also live in robust.R.
wedge_yearly_outputs <- lapply(seq_len(nrow(outcome_groups)), function(i) {
    ...
    list(
        year = write_year_output(
            outcomes,
            paste0("wedge_year_by_year_2x2_", outcome_group),
            paste0("Wedge Year-by-Year ", outcome_label, " Establishment Models"),
            "Estimate"
        ),
        tax_gap = write_tax_gap_output(
            outcomes,
            paste0("wedge_tax_gap_nonlinear_by_year_", outcome_group),
            paste0("Wedge ", outcome_label, " High-Tax Effects by Sales Tax Gap"),
            "Estimate"
        )
    )
})
names(wedge_yearly_outputs) <- outcome_groups$outcome_group
